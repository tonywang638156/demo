...
CHROMADB_PATH = os.getenv("VECTOR_DATABASE_PATH", "./ts-cm-db")
...
def generate_timesheet_db(df, collection):
    """
    Enhance timesheet comments using LLM (with caching), embed them, and store them in ChromaDB.
    ...
    existing_ids = set(doc_id for sublist in existing_docs["ids"] for doc_id in sublist)
    ...
    for i, row in df.iterrows():
        ...
        # Get original comment and enrich it
        short_comment = str(row.get("trn_desc", "")).strip()
        enriched_comment = expand_comment_with_llm(short_comment, expanded_cache)
        # Compose a combined text for embedding
        doc_text = (
            f"Original Comment: {short_comment}\n"
            f"Enriched Comment: {enriched_comment}\n"
            f"Project Code: {row.get('prj_code', '')}\n"
            f"Project Name: {row.get('prj_name', '')}"
        )

        # Get embedding for the enriched comment (or you may choose to embed doc_text)
        embed_response = ollama.embeddings(model="mxbai-embed-large", prompt=enriched_comment)
        ...
        collection.add(
            ...
            metadatas=[{
                ...
                "enriched_comment": enriched_comment,
    ...
    print("Timesheet database has been updated with enriched descriptions.")
