...
# --- Configuration ---
TIMESHEET_FILEPATH = os.getenv("TIMESHEET_FILEPATH", "./clean.xlsx")  # e.g. "clean.xlsx"
CHROMADB_PATH = os.getenv("VECTOR_DATABASE_PATH", "./ts-cm-db")      # e.g. "timesheet-chroma"
COLLECTION_NAME = "expanded-embeddings-a"
...
def generate_timesheet_db(df, collection):
    ...
    for i, row in df.iterrows():
        ...
        print("")
        print(f"Expanded Comment for {doc_id}: {expanded_comment}", end = " ")  
...
def main():
    ...
    # 4) Test a query
    test_query = "background"  # or "interior design"
    results = query_timesheet_db(test_query, collection, top_n=5)
